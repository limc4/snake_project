...
def game_loop():
    ...
    while not quit_game:  # loop to keep game running unless quit
        ...
        for event in pygame.event.get():  # receives all events from user inputs
            ...
            # checks to see which key the user has pressed using K_LEFT and
            # K_Right to handle snake movement
            if event.type == pygame.KEYDOWN:
        ...
        screen.fill(green)  # changes screen (surface) from default black to
        # green

        # create snake (replaces simple rectangle in previous version
        snake_head = [snake_x, snake_y]
        snake_list.append(snake_head)  # snake_head is nested inside snake_list
        if len(snake_list) > snake_length:
            del snake_list[0]

        for x in snake_list[:-1]:  # detects if snake_head touches any other
            # part (-1 is used to count from the end of the list)
            if x == snake_head:
                game_over = True
        ...
        # detect snake contact with food sprite (instead of circle)
        if snake_x == food_x and snake_y == food_y:
            # set new random position for food if snake touches it
            food_x = round(random.randrange(20, 1000 - 20) / 20) * 20
            food_y = round(random.randrange(20, 720 - 20) / 20) * 20
            print("Got it!")  # for testing purposes
